   ...
   .then((response) => {
    ...
    // .then((formData)=> {setView(formData); })
     ...
     console.log(view);
      ...
      //   fetch('http://localhost:8080/OverTime/bydate',options)
      //     .then(response => response.json())
      //     .then(data => setFormData(formData));
      // }, []);
       return (
         <>
         <form onSubmit={handleSubmit}>
           ...
           <div> <label>End Date:</label>
            <input type="text" name="enddate" value={formData.enddate} onChange={handleChange} /></div>
            <button type="submit">Send to Postman</button> </form> 
            <MaterialTable 
            columns={
              [ 
                { title: "ID", field: "id", },
